<?phpfunction readFilesFromFolder($folder)
{
return array_diff(scandir($folder), array('..', '.'))}function imageFromFolder($folder, $tags, $slideToIndex)
{
...
echo "<imgsrc='${folder}/${imagem}' alt='' title=''><span";
echo "class='icon-focus'></span></div>";
echo "</div>";
$slideToIndex++}
return $slideToIndex}function imageModalFromFolder($folder, $firstList)
{
$imagens = readFilesFromFolder($folder);
$index = 1;
foreach ($imagens as $imagem){
if ($index == 1 && $firstList){
echo "<div class='carousel-item active'>"} else{
echo "<div class='carousel-item'>"}
echo "<imgsrc='${folder}/${imagem}' alt='' title=''>";
echo "</div>";
$index++}}?> <header class="masthead"> <div class="container"> <div class="intro-text"> <div class="intro-heading text-uppercase"style="color:#fec503;">Portfólio</div> </div> </div> </header>
...
</div><div data-app-prevent-settings=""class="mbr-slider modal fade carousel slide"tabindex="-1"data-keyboard="true"data-interval="false"id="lb-gallery2-0">
